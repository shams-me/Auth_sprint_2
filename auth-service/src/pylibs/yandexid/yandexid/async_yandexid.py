...
class AsyncYandexID:
    ...
    def __init__(self, oauth_token: str, client: AsyncClient | None = None):
        ...
        self.__client = client or AsyncClient(headers=self.__headers, base_url=self.BASE_URL)
    ...
    async def get_user_info(self, format, jwt_secret: str | None = None, with_openid_identity: bool = False) -> str:
        """Get user info
    ...
    async def get_user_info_json(self, with_openid_identity: bool = False) -> User:
        ...
        Returns:
            User: User info
        """
        data = await self.get_user_info(format="json", with_openid_identity=with_openid_identity)
    ...
    async def get_user_info_xml(self, with_openid_identity: bool = False) -> str:
        ...
        Returns:
            str: User info
        """
        data = await self.get_user_info(format="xml", with_openid_identity=with_openid_identity)
        return data
    ...
    async def get_user_info_jwt(
        ...
        data = await self.get_user_info_jwt_unparsed(jwt_secret=jwt_secret, with_openid_identity=with_openid_identity)
