...
from ..validators import DeviceID, DeviceName, OptionalScope
...
class YandexOAuth:
    ...
    def __init__(
        ...
        self.__headers = {"User-Agent": f"YandexID/{__version__}"}
        self.__params = {"client_id": client_id}
        self.__client = client or Client(base_url=self.BASE_URL, params=self.__params, headers=self.__headers)
            ...
            try:
                ...
                if not device_name:
                    warnings.warn(
                        ...
                        "Yandex ID returns token for unknown device.",
                        UserWarning,
                    )
        if device_name:
            try:
                ...
                if not device_id:
                    warnings.warn(
                        "device_name is specified, but device_id is not. " "device_name will be ignored.",
                        UserWarning,
                    )
        if login_hint:
    ...
    def get_token_from_code(self, code: str, device_id: str | None = None, device_name: str | None = None) -> Token:
        """Get token from code
        ...
        headers = {"Authorization": f'Basic {b64encode(f"{self._client_id}:{self._client_secret}".encode()).decode()}'}
